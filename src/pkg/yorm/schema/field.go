...
func (schema *Schema) ParseField(fieldStruct reflect.StructField) *Field {
	...
	field := &Field{
		...
		Readable:          true,
		Tag:               fieldStruct.Tag,
		TagSettings:       ParseTagSetting(fieldStruct.Tag.Get("yorm"), ";"),
		Schema:            schema,
	}
		...
		getRealFieldValue = func(v reflect.Value) {
			...
			if rv.Kind() == reflect.Struct && !rv.Type().ConvertibleTo(TimeReflectType) {
				for i := 0; i < rv.Type().NumField(); i++ {
					...
					for key, value := range ParseTagSetting(field.IndirectFieldType.Field(i).Tag.Get("yorm"), ";") {
	...
	case reflect.Struct:
		if _, ok := fieldValue.Interface().(*time.Time); ok {
			field.DataType = Time
			field.IndirectFieldType = reflect.TypeOf("")
	...
	if _, ok := field.TagSettings["EMBEDDED"]; ok || (fieldStruct.Anonymous && !isValuer && (field.Creatable || field.Updatable || field.Readable)) {
		if reflect.Indirect(fieldValue).Kind() == reflect.Struct {
			...
			if field.EmbeddedSchema, err = Parse(fieldValue.Interface(), cacheStore); err != nil {
				schema.err = err
			}
